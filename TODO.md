...
- [ ] **I1** — `apps/console-api/src/services/sim-swarm-store.service.ts:56-71, 102-112, 135-138` — move raw `db.transaction()` / `db.update(simSwarm)` / `db.execute(sql)` into `SimSwarmRepository` / `SimRunRepository` methods.
...
- [ ] **I8 — kernel reads `process.env` in 20+ locations (DIP)** — `packages/thalamus/src/explorer/crawler.ts:138,243` + `nano-caller.ts:197,275,288,389` + `utils/voyage-embedder.ts:27` + `config/enrichment.ts:14-41` (10 reads) + `cortices/sources/fetcher-celestrak.ts:42` + `fetcher-launch-market.ts:17` + `fetcher-spectra.ts:17`. Extend `setNanoConfigProvider` / `ConfigProvider<T>` pattern to cover all of them; inject from console-api container at boot.
