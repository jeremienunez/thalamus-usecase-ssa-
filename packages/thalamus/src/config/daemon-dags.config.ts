/**
 * Predefined daemon-trigger DAGs (no LLM call needed).
 *
 * Cortex keys track the SSA `ResearchCortex` enum values (snake_case).
 * Extracted from `thalamus-planner.service.ts` so adding a daemon job no
 * longer requires editing planner code (Open/Closed principle). The planner
 * accepts a `daemonDags` map via constructor injection; this export is the
 * default wiring.
 */
