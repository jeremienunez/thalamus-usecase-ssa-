/**
 * Legacy SSA resolution handlers — duplicated from
 * apps/console-api/src/agent/ssa/sweep/resolution-handlers.ssa.ts so the
 * sweep container has a default `ResolutionHandlerRegistry` when
 * opts.ports.resolutionHandlers isn't supplied.
 *
 * Phase 4 collapses the duplication once console-api becomes the sole
 * wiring path. Until then this file mirrors the console-api version
 * byte-for-byte modulo imports.
 */
...
export function createLegacySsaResolutionRegistry(
  deps: SsaHandlerDeps,
): ResolutionHandlerRegistry {
