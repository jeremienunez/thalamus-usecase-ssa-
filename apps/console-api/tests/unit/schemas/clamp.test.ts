...
describe("clampedInt", () => {
  ...
  it("clampedInt uses its default when the value is omitted", () => {
    expect(schema.parse(undefined)).toBe(3);
  });

  it("accepts integer-like query strings and clamps them into range", () => {
    expect(schema.parse("0")).toBe(1);
    expect(schema.parse(" 4 ")).toBe(4);
    expect(schema.parse(9)).toBe(5);
  });

  it("rejects decimal strings, blank strings, non-finite values, and booleans", () => {
    for (const value of [
      1.5,
      "1.5",
      "",
...
describe("clampedNumber", () => {
  ...
  it("clampedNumber uses its default when the value is omitted", () => {
    expect(schema.parse(undefined)).toBe(0.5);
  });

  it("clampedNumber accepts decimal strings and clamps into the float range", () => {
    expect(schema.parse("0.01")).toBe(0.1);
    expect(schema.parse(" 0.25 ")).toBe(0.25);
    expect(schema.parse(1.2)).toBe(0.9);
  });

  it("rejects blank strings, non-finite values, and booleans", () => {
