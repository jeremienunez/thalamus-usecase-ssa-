...
// ─── Cycle launcher — appends synthetic findings to an existing fixture ─────
...
/**
 * Generate N synthetic findings as if a research cycle just completed and
 * mutate the fx.findings array in place. Returns a Cycle descriptor so the
 * UI can render a "launched" toast + cycle history.
 */
export function launchCycle(
  fx: ReturnType<typeof buildFixtures>,
  kind: CycleKind,
): Cycle {
  const id = `cyc:${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`;
  ...
  const cortices =
    kind === "thalamus"
      ...
      : kind === "fish"
        ? FISH_CORTICES
        : [...THALAMUS_CORTICES, ...FISH_CORTICES];
  const nFindings =
    kind === "both" ? 8 + Math.floor(Math.random() * 6) : 4 + Math.floor(Math.random() * 4);

  const sats = fx.satellites;
  for (let i = 0; i < nFindings; i++) {
    const cortex = cortices[Math.floor(Math.random() * cortices.length)]!;
    const sat = sats[Math.floor(Math.random() * sats.length)]!;
    const suffix = id.slice(-4);
    const title =
      cortex === "opacity-scout"
        ? `Public-data gap on ${sat.name} — 2 amateur sources corroborate`
        : cortex === "conjunction-analysis"
          ? `Conjunction window +${1 + Math.floor(Math.random() * 48)}h · ${sat.name}`
          : cortex === "correlation"
            ? `OSINT lane disagrees with catalog on ${sat.name}`
            : cortex === "catalog"
              ? `Stale TLE epoch · ${sat.name}`
              : cortex === "data-auditor"
                ? `Null operator_country on ${sat.name}`
                : cortex === "classification-auditor"
                  ? `Declared label drift for ${sat.name}`
                  : `Finding · ${cortex} · ${sat.name}`;
    fx.findings.unshift({
      id: `f:${suffix}-${i + 1}`,
      title,
      summary: `Produced by ${kind.toUpperCase()} cycle ${id}. Cortex ${cortex} observed evidence warranting reviewer attention.`,
      cortex,
      status: "pending",
      priority: 50 + Math.floor(Math.random() * 50),
      createdAt: new Date().toISOString(),
      linkedEntityIds: [`sat:${sat.id}`],
      evidence:
        cortex === "opacity-scout"
          ? [
              {
                kind: "osint",
                uri: "https://satobs.org/seesat/Apr-2026/msg00099.html",
                snippet: "Molczan observation — reconstructed TLE",
              },
            ]
          : [
              {
                kind: "osint",
                uri: "celestrak://active",
                snippet: "Element set drift",
              },
            ],
    });
  }
